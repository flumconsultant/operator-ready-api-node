...
preparar.parameters.assignments.assignments = [
  ...
  { id: 'pi-prov', name: 'trama_index_provisional', value: '={{ $("Code TRAMA Index").first().json.trama_index_provisional }}', type: 'boolean' },
  {
    id: 'pi-resp',
    name: 'respuestas',
    /* A la IA se le mandaba «pregunta_7: 4». Ahora va el texto de la pregunta y
       la respuesta tal como la eligió la persona, que es lo que puede
       interpretar. Si las preguntas siguen sin redactar, al menos ve la
       etiqueta elegida en vez de un número suelto. */
    value: `={{ Object.fromEntries(${CFG}.preguntas.map((p) => [p.texto, $json[p.id]]).filter(([, r]) => r !== undefined && r !== '')) }}`,
    type: 'object',
  },
];
